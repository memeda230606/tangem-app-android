...
本项目选定客户端分片加密备份作为默认方案：手机生成随机 256 位恢复包装密钥，用它加密钱包恢复载荷，再将恢复包装密钥按 GF(256) Shamir 2-of-3 拆分。三份分别由客户恢复码、绑定卡的受保护 File 03、服务器 HSM/KMS 持有。分片对象不是助记词；服务器只保存恢复包密文和经独立 KEK 包装的服务器分片。客户恢复码与绑定卡为默认恢复组合，任何使用服务器分片的路径都必须重新认证、执行风控并留下双人审批审计。
